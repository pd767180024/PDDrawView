...
- (void)createLabel
{
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 1, self.bounds.size.width, self.bounds.size.height)];
    label.font = [UIFont fontWithName:@"MyriadPro-Bold" size:12];
...
-(void)layoutSubviews{
    ...
    _trackLayer.frame=self.bounds;
    _progressLayer.frame=self.bounds;
    _textLabel.frame = CGRectMake(0, 1, self.bounds.size.width, self.bounds.size.height);
...
-(void)setProgressColor:(UIColor *)progressColor{
    ...
    _textLabel.textColor = progressColor;
...
-(void)setProgressLineWidth:(CGFloat)progressLineWidth{
    ...
    CGFloat radius = self.frame.size.width/2.0-_progressLineWidth;
    UIBezierPath *path=[UIBezierPath bezierPathWithArcCenter:CGPointMake(self.frame.size.width/2.0, self.frame.size.height/2.0) radius:radius startAngle:CATDegreesToRadians(_startAngle) endAngle:CATDegreesToRadians(_endAngle) clockwise:YES];//-210到30的path
